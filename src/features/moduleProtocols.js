...
const groupToCurriculum = { 'TA-20E': 3127 };
...
async function getGroupProtocols(group) {
  let groupProtocols = await getJson(
    `https://tahvel.edu.ee/hois_back/moduleProtocols?isVocational=true&curriculumVersion=${groupToCurriculum[group]}&lang=ET&page=0&size=50&studentGroup=${groupsMap[group]}`
  );
  let groupModules = await getJson(
    `https://tahvel.edu.ee/hois_back/moduleProtocols/occupationModules/${groupToCurriculum[group]}`
  );
